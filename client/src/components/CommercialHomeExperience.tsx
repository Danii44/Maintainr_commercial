...
function TiltCard({ children }: { children: ReactNode }) {
  ...
  const handleMove = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    const x = ((event.clientY - rect.top) / rect.height - .5) * -7;
    const y = ((event.clientX - rect.left) / rect.width - .5) * 9;
    setRotation({ x, y });
  };
  return <div className="marketing-tilt-shell" onPointerMove={handleMove} onPointerLeave={() => setRotation({ x: 0, y: 0 })} style={{ transform: `perspective(900px) rotateX(${rotation.x}deg) rotateY(${rotation.y}deg)` }}>{children}</div>;
}
...
export function CommercialHomeExperience() {
  ...
  return <main className="maintainr-marketing-home">
    ...
    <section className="marketing-section marketing-problem-section"><div className="marketing-container"><Reveal className="marketing-section-intro"><Eyebrow>{t("WHEN WORK GETS LOST", "عندما يضيع العمل")}</Eyebrow><h2>{t("The work should move forward—not disappear between messages.", "يجب أن يتقدم العمل، لا أن يختفي بين الرسائل.")}</h2><p>{t("Maintainr brings the detail, responsibility, updates, and evidence into a workflow people can follow without rebuilding context at every handoff.", "يجمع Maintainr التفاصيل والمسؤولية والتحديثات والإثبات في مسار عمل يمكن للناس اتباعه من دون إعادة بناء السياق عند كل تسليم.")}</p></Reveal><div className="marketing-problem-grid">{friction.map(({ icon: Icon, number, title, body }, index) => <Reveal key={title} delay={index * .08}><TiltCard><article className="marketing-problem-card"><span className="marketing-card-icon"><Icon size={21} /></span><p className="marketing-card-number">{number}</p><h3>{title}</h3><p>{body}</p></article></TiltCard></Reveal>)}</div></div></section>

    <section id="how-it-works" className="marketing-section marketing-workflow-section"><div className="marketing-container"><Reveal className="marketing-section-intro marketing-section-intro-center"><Eyebrow>{t("HOW MAINTAINR GUIDES THE WORK", "كيف يوجّه Maintainr العمل")}</Eyebrow><h2>{t("A clear operating rhythm for every request.", "إيقاع تشغيلي واضح لكل طلب.")}</h2><p>{t("Each stage answers the questions that slow teams down: what happened, who owns it, what is next, and what proves it is complete?", "تجيب كل مرحلة عن الأسئلة التي تبطئ الفرق: ماذا حدث، من المسؤول، ما الخطوة التالية، وما الذي يثبت اكتماله؟")}</p></Reveal><div className="marketing-workflow-grid">{guide.map(({ icon: Icon, step, title, body }, index) => <Reveal key={title} delay={index * .06}><TiltCard><article className="marketing-workflow-card"><p>{step}</p><span><Icon size={22} /></span><h3>{title}</h3><p>{body}</p></article></TiltCard></Reveal>)}</div></div></section>

    <section className="marketing-proof-dark"><div className="marketing-container"><Reveal className="marketing-section-intro marketing-section-intro-center"><Eyebrow inverse>{t("BUILT FOR THE WORK AHEAD", "مصمم للعمل القادم")}</Eyebrow><h2>{t("The operating record your team can trust.", "السجل التشغيلي الذي يثق به فريقك.")}</h2><p>{t("Teams do better work when the record is clear, the next step is visible, and the outcome can be reviewed later.", "تعمل الفرق بشكل أفضل عندما يكون السجل واضحاً والخطوة التالية مرئية ويمكن مراجعة النتيجة لاحقاً.")}</p></Reveal><div className="marketing-testimonial-grid">{testimonials.map(({ quote, name, detail }, index) => <Reveal key={name} delay={index * .08}><TiltCard><blockquote className="marketing-testimonial"><Quote size={22} /><p>“{quote}”</p><footer><strong>{name}</strong><span>{detail}</span></footer></blockquote></TiltCard></Reveal>)}</div></div></section>
    ...
    <section id="pricing" className="marketing-pricing-section"><div className="marketing-container"><Reveal className="marketing-section-intro marketing-section-intro-center"><Eyebrow>{t("A CLEAR PLACE TO START", "مكان واضح للبدء")}</Eyebrow><h2>{t("A workspace shaped around your operation.", "مساحة عمل مصممة حول عملياتك.")}</h2><p>{t("Every plan starts with the same principle: keep the work, responsibility, and proof connected. Talk with us to shape the right workspace for your properties.", "تبدأ كل خطة بالمبدأ نفسه: إبقاء العمل والمسؤولية والإثبات متصلة. تحدث معنا لتشكيل مساحة العمل المناسبة لعقاراتك.")}</p></Reveal><div className="marketing-pricing-grid">{plans.map(({ name, detail, price, points }, index) => <Reveal key={name} delay={index * .07}><TiltCard><article className={index === 1 ? "marketing-price-card marketing-price-card-featured" : "marketing-price-card"}>{index === 1 && <span className="marketing-price-badge">{t("MOST FLEXIBLE", "الأكثر مرونة")}</span>}<Eyebrow>{name}</Eyebrow><p className="marketing-price-detail">{detail}</p><div className="marketing-price-rule" /> <ul>{points.map(point => <li key={point}><CheckCircle2 size={16} />{point}</li>)}</ul><Primary href="/quote"><Quote size={16} />{price}</Primary></article></TiltCard></Reveal>)}</div></div></section>

    <section className="marketing-faq-section"><div className="marketing-container marketing-faq-wrap"><Reveal className="marketing-section-intro marketing-section-intro-center"><Eyebrow>{t("PRACTICAL QUESTIONS", "أسئلة عملية")}</Eyebrow><h2>{t("Questions teams ask before they switch.", "أسئلة تطرحها الفرق قبل الانتقال.")}</h2></Reveal><div className="marketing-faq-list">{questions.map(([question, answer], index) => <Reveal key={question} delay={index * .05}><details><summary>{question}<ArrowRight size={18} /></summary><p>{answer}</p></details></Reveal>)}</div></div></section>

    <section className="marketing-final-cta"><div className="marketing-container"><Reveal><div><Eyebrow inverse>{t("READY TO MAKE THE WORK EASIER TO RUN?", "هل أنت جاهز لجعل العمل أسهل في الإدارة؟")}</Eyebrow><h2>{t("Make the next maintenance handoff the easy one.", "اجعل تسليم الصيانة التالي هو الأسهل.")}</h2><p>{t("Tell us how maintenance moves through your portfolio. We will help you shape a workflow that keeps every handoff connected.", "أخبرنا كيف تتحرك الصيانة عبر محفظتك. سنساعدك على تشكيل مسار عمل يبقي كل تسليم متصلاً.")}</p></div><Primary href="/quote"><Quote size={17} />{t("Plan your workflow", "خطط لمسار عملك")}</Primary></Reveal></div></section>
  </main>;
}
