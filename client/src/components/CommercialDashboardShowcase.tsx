...
export function CommercialDashboardShowcase({ tone = "home" }: { tone?: ShowcaseTone }) {
  ...
  return <section id="dashboard-showcase" className="maintainr-dashboard-showcase relative overflow-hidden bg-[#eaf8f5] px-5 py-24 lg:px-8">
    ...
    <div className="relative mx-auto max-w-7xl">
      ...
      <div className="mt-14 grid gap-10 lg:grid-cols-[1.22fr_.78fr] lg:items-center">
        <motion.div initial={{ opacity: 0, x: reduce ? 0 : -34 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, amount: .16 }} transition={{ duration: .72, ease: [0.23, 1, .32, 1] }} className="relative">
          <motion.div animate={reduce ? undefined : { y: [0, -7, 0] }} transition={{ duration: 5.2, repeat: Infinity, ease: "easeInOut" }} className="relative overflow-hidden rounded-[2rem] border border-[#a9ded6] bg-white p-2 shadow-[0_28px_70px_rgba(15,118,110,.16)]">
            <img src="/images/maintainr-dashboard-manager.png" alt={t("Maintainr manager dashboard showing maintenance requests and priorities", "لوحة تحكم مدير Maintainr تعرض طلبات الصيانة والأولويات")} className="block aspect-[16/10] w-full rounded-[1.55rem] object-cover object-top" />
            <div className="absolute left-7 top-7 hidden items-center gap-2 rounded-full bg-white/95 px-3 py-2 text-[10px] font-bold tracking-[.14em] text-[#0f766e] shadow-lg sm:flex"><Sparkles size={13} />{t("LIVE WORKSPACE", "مساحة عمل مباشرة")}</div>
            <motion.div initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: .45, duration: .42 }} className="absolute bottom-6 left-6 hidden max-w-56 rounded-2xl border border-white/70 bg-[#0b756d] p-3 text-white shadow-2xl sm:block"><div className="flex items-center gap-2 text-xs font-semibold"><BellRing size={15} />{t("Follow-up stays visible", "المتابعة تبقى مرئية")}</div><p className="mt-1 text-[11px] leading-4 text-teal-100">{t("Reminders and updates stay with the request.", "تبقى التذكيرات والتحديثات مع الطلب.")}</p></motion.div>
          </motion.div>
          <motion.div initial={{ opacity: 0, scale: .94, x: reduce ? 0 : 18 }} whileInView={{ opacity: 1, scale: 1, x: 0 }} viewport={{ once: true, amount: .2 }} transition={{ delay: .18, duration: .55, ease: [0.23, 1, .32, 1] }} className="absolute -bottom-10 -right-4 hidden w-44 overflow-hidden rounded-[1.7rem] border-4 border-white bg-[#0b0f18] p-1 shadow-[0_24px_55px_rgba(23,32,51,.28)] md:block lg:-right-10">
            <img src="/images/maintainr-dashboard-mobile.png" alt={t("Maintainr mobile dashboard navigation", "تنقل لوحة تحكم Maintainr على الهاتف")} className="block w-full rounded-[1.3rem]" />
          </motion.div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, x: reduce ? 0 : 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, amount: .16 }} transition={{ duration: .68, ease: [0.23, 1, .32, 1] }}>
          <div className="rounded-[1.75rem] border border-[#b9e7df] bg-white/85 p-6 shadow-[0_18px_42px_rgba(23,32,51,.07)] backdrop-blur sm:p-8">
      ...
      <motion.div initial={{ opacity: 0, y: reduce ? 0 : 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: .25 }} transition={{ delay: .08, duration: .55 }} className="mt-16 grid gap-3 md:grid-cols-3">
        {[[t("One calm home", "موطن هادئ واحد"), t("Requests, messages, reminders, and next actions stay easy to find.", "تبقى الطلبات والرسائل والتذكيرات والإجراءات التالية سهلة العثور عليها.")], [t("Fewer manual follow-ups", "متابعات يدوية أقل"), t("A connected record makes it easier to keep people informed without rebuilding context.", "يسهّل السجل المتصل إبقاء الأشخاص على اطلاع دون إعادة بناء السياق.")], [t("Ready for the real day", "جاهز لليوم الحقيقي"), t("Use the same workspace in English or Arabic, at the desk or in the field.", "استخدم مساحة العمل نفسها بالإنجليزية أو العربية، على المكتب أو في الميدان.")]].map(([title, body]) => <article key={title} className="rounded-2xl border border-[#b9e7df] bg-white/65 p-5"><p className="text-sm font-semibold text-[#172033]">{title}</p><p className="mt-2 text-sm leading-6 text-[#526176]">{body}</p></article>)}
      </motion.div>
    </div>
  </section>;
}
