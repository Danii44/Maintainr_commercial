...
function ThemeToggle({ compact = false }: { compact?: boolean }) {
  ...
  return <button type="button" onClick={toggleTheme} aria-label={label} aria-pressed={dark} title={label} className={`maintainr-theme-toggle maintainr-theme-toggle-dark-safe inline-flex items-center justify-center gap-2 rounded-full border text-xs font-semibold transition active:scale-[.97] ${compact ? "size-10" : "min-h-10 px-3"}`}>
