...
export function CommercialHeader({ current }: { current: string }) {
  ...
  return <header className="relative z-50 border-b border-slate-200/80 bg-white/85 backdrop-blur-xl dark:border-white/[.07] dark:bg-[#080a12]/80">
    <div className="mx-auto flex h-[76px] max-w-7xl items-center justify-between px-5 lg:px-8">
      <a href="/" className="inline-flex items-center gap-2 font-semibold tracking-tight text-slate-950 dark:text-white"><span className="grid size-9 place-items-center rounded-xl bg-gradient-to-br from-[#0e4c46] to-[#4f8378] text-white shadow-lg shadow-[#0e4c46]/20"><Wrench size={18}/></span><span>Maintainr</span></a>
      ...
      <div className="hidden items-center gap-2 lg:flex"><button onClick={toggleLanguage} className="rounded-lg px-2.5 py-2 text-xs font-semibold text-slate-600 transition hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-white/[.06]">{direction === "rtl" ? "EN" : "ع"}</button><a href="/quote?intent=demo" className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-[#b6d7cd] bg-[#eff6f3] px-3 text-xs font-semibold text-[#0e4c46] transition hover:bg-[#e0eee8] dark:border-[#b6d7cd]/25 dark:bg-[#b6d7cd]/10 dark:text-[#b6d7cd] dark:hover:bg-[#b6d7cd]/20"><CalendarClock size={14}/>{t("Request demo", "اطلب عرضاً")}</a><a href="/quote?intent=quote" className="maintainr-primary-action inline-flex min-h-10 items-center gap-2 rounded-xl bg-[#0e4c46] px-4 text-sm font-semibold text-white shadow-lg shadow-[#0e4c46]/20 transition hover:-translate-y-0.5 hover:bg-[#083c38] active:scale-[.97] dark:bg-[#d9bc82] dark:text-[#173f3b] dark:hover:bg-[#ebd4a6]"><Quote size={15}/>{t("Request quote", "اطلب عرض سعر")}</a></div>
